...
namespace Tyvj.Helpers
{
    public static class Ratings
    {
        ...
        public static void RatingCount(int id)//contest_id
        {
            ...
            var problems = (from p in DbContext.ContestProblems
                            where p.ContestID == id
                            orderby p.Point ascending
                            select p.Problem).ToList();
            #endregion
            for (int i = 0; i < ranklist.Count; i++)
            {
                ...
                foreach (var detail in ranklist[i].Details)
                {
                    ...
                    else if (contest.Format == ContestFormat.Codeforces)
                    {
                        ...
                        if (detail.Key1 > 0)
                        {
                            user_credits += base_pts + (int)((Math.Floor((extend_pts + 1) / 30)) * (problems[num].Difficulty - detail.Key1));
                        }
                        else
                        {
                            user_credits += base_pts + (int)((Math.Floor((extend_pts + 1) / 25)));
                ...
                DbContext.Ratings.Add(new Tyvj.DataModels.Rating
                {
                    Credit = user_credits,
                    ContestID = id,
                    Time = contest.End,
